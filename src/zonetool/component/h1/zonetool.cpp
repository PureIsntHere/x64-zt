...
namespace zonetool::h1
{
    ...
    static void register_commands()
    {
        ...
        ::h1::command::add("batchdumpzone", [](const ::h1::command::params& params) {
            ...
            for (auto& z : zones) {
                auto name = z.stem().string();  // strip “.ff”
                if (name == "hmw_launcher" || name == "hmw_launcher_mp" || name == "patch_common_mp") {
                    ZONETOOL_ERROR("Skipping launchers zone \"%s\" in batchdumpzone", name.c_str());
                    continue;
				}
        ...
        ::h1::command::add("batchdumpzonewalk", [](const ::h1::command::params& params) {
            ...
            for (auto& z : zones) {
                auto name = z.stem().string();  // strip “.ff”
                if (name == "hmw_launcher" || name == "hmw_launcher_mp" || name == "patch_common_mp") {
                    ZONETOOL_ERROR("Skipping launchers zone \"%s\" in batchdumpzonewalk", name.c_str());
                    continue;
                }
                ZONETOOL_INFO("Batch dumping zone \"%s\"", name.c_str());
                ::h1::command::execute("dumpzone " + name, true);
    ...
    public:
        void post_load() override
        {
            ...
            // disable splash, WMI, demonware, etc.
            utils::hook::set<uint8_t>(0x140513840, 0xC3);
            utils::hook::set<uint8_t>(0x140543730, 0xC3);
            utils::hook::set<uint8_t>(0x140046588, 0xC3);
            utils::hook::set<uint8_t>(0x14009CA40, 0xC3);
